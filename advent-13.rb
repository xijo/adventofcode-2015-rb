...
input.each_line do |line|
  mod = line.include?('lose') ? -1 : 1
  first, last = line.sub('.', '').split.rotate(-1).first(2).reverse
  sympathies[first] ||= {}
  sympathies[first][last] = line[/\d+/].to_i * mod
end

guests = sympathies.keys

best_rank = 0

guests.permutation.each do |arrangement|
  value = 0
  arrangement.each_with_index do |guest, i|
    left = arrangement[i-1]
    right = arrangement[i == (arrangement.size - 1) ? 0 : i+1]
    value += sympathies[guest][left]
    value += sympathies[guest][right]
  end
  best_rank = value if value > best_rank
end
